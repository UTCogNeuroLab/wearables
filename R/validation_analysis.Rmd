...
corrdf <- dfcor %>%
  group_by(subject) %>%
  summarise(sleep_duration_r = cor(sleep_duration_act, sleep_duration_fit, method = 'spearman', use = "everything"), 
            time_in_bed_r = cor(time_in_bed_act, time_in_bed_fit, method = 'spearman'))
  
dfcor$sleep_duration_fit
...
```
```{r}
d1 <- read_csv(paste0(data_dir, 'dataset_2021-08-06.csv'))
d2 <- read_csv(paste0(data_dir, "sleep_diaries/sleep_diaries_all.csv"))

d2$sleep_quality <- recode(d2$sleep_quality, "Very poor" = '1', 'Poorly' = '2', 'Mediocre' = '3', 'Well' = '4', 'Very well' = '5')
d2$sleep_quality <- as.numeric(d2$sleep_quality)

d2 <- d2 %>%
  select(record_id, sleep_quality) %>%
  group_by(record_id) %>%
  summarise(sleep_quality_mean = mean(sleep_quality, na.rm = T), sleep_quality_sd = sd(sleep_quality, na.rm = T))
  
d3 <- merge(d1, d2, by.x = 'subject', by.y = 'record_id')
...
d3 %>%
  filter(age_group == 1) %>%
  select(contains('7_fit_norm'), mean) %>%
  mutate_all(as.numeric) %>%
  corrr::correlate() %>%
  corrr::rplot() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
